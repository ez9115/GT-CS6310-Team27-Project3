					...
					while (!mRunningThread.isInterrupted()) {
						...
						if (!stabilizationAchieved) {
							...
							if (minMaxTemp != null && !stabilizationAchieved) {
								if (Math.abs(newMinMaxTemp.Max - minMaxTemp.Max) <= STABILIZATION_DELTA
										&& Math.abs(newMinMaxTemp.Min
												- minMaxTemp.Min) <= STABILIZATION_DELTA) {
									System.out
						...
						if (stabilizationAchieved) {
							System.out.println("Stabilization achieved at "
									+ numberOfIterationsToStabilization
									+ " iterations");
						}
						...
						sunPosition = Utils.incrementSunPosition(sunPosition,
								mTimeStep);
					}
