...
/**
 * PausableStoppable provides some core functionality for classes with concurrent actions
 * that can be started, stopped, paused, and resumed.
...
public abstract class PausableStoppable {
	...
	/**
	 * Stores a reference to the currently executing thread.
	 * Once the thread is stopped, this variable is set back to null.
	 */
	protected Thread mRunningThread;
	
	/**
	 * Holds the current pause state. If true, the thread should pause at the next
	...
	/**
	 * Resumes the underlying thread if it is paused. Throws an exception if the thread has not been started.
	 * @throws Exception Thrown if the thread has not been started.
	 */
	...
	/**
	 * Starts the the Runnable from getRunnableAction() on a new thread and begins the
	...
	/**
	 * Stops the currently executing thread. Throws an exception if the thread has not been started.
	 * @throws Exception Thrown if the thread has not been started.
	 */
	...
	/**
	 * Should be overridden to return a Runnable to execute in the thread when start() is called.
