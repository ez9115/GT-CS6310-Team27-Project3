...
public abstract class ObjectFactory {
	...
	public static PausableStoppable getInitiative(
			InitiativeType initiativeType, int bufferSize,
			boolean asyncSimulation, boolean asyncPresentation,
			PresentationMethod presentationMethod) throws Exception {
		switch (initiativeType) {
		case MasterController:
			return masterController(presentationMethod, bufferSize,
					asyncSimulation, asyncPresentation);
		case Presentation:
			return presentationInitiative(presentationMethod, bufferSize,
					asyncSimulation, asyncPresentation);
		case Simulation:
			return simulationInitiative(presentationMethod, bufferSize,
					asyncSimulation, asyncPresentation);
		default:
	...
	private static PausableStoppable masterController(
			PresentationMethod presentation, int bufferSize,
			boolean asyncSimulation, boolean asyncPresentation) {
		return new MasterControllerInitiative(bufferSize, asyncSimulation,
				asyncPresentation, getSimulationMethod(), presentation);
	}

	private static PausableStoppable presentationInitiative(
			PresentationMethod presentationMethod, int bufferSize,
			boolean asyncSimulation, boolean asyncPresentation) {

		BlockingQueue<SimulationResult> queue = new ArrayBlockingQueue<SimulationResult>(
				bufferSize);
		PresentationInitiative presentation = new PresentationInitiative(queue,
				presentationMethod);
		final SimulationInitiative simulation = new SimulationInitiative(queue,
				getSimulationMethod());
		...
		try {
			OnStart starter = Utils.startSimulationProcess(asyncPresentation,
					asyncSimulation, queue, presentation, simulation);
	...
	private static PausableStoppable simulationInitiative(
		...
		BlockingQueue<SimulationResult> queue = new ArrayBlockingQueue<SimulationResult>(
				bufferSize);
		final PresentationInitiative presentation = new PresentationInitiative(
				queue, presentationMethod);
		SimulationInitiative simulation = new SimulationInitiative(queue,
				getSimulationMethod());
