...
public class CellpropertiesBuilder {
	...
	void setNeighbors(int num, CellpropertiesBuilder cell) {
		...
		System.out.println("We are in neighbor[ num ].p="
				+ neighbor[num].cell.T + "     nun =" + num);
	}
	...
	void init(int i_lat, int i_lon, int cols, int rows, int gs,
		...
		l_b = Math.cos(lat * PI / 180.0) * l_v; // length of the base of the
												// cell
		l_t = Math.cos((lat + gs) * PI / 180.0) * l_v; // length of the top of
														// the cell
		h = Math.sqrt(l_v * l_v - 0.25 * (l_b - l_t) * (l_b - l_t)); // height
																		// of
																		// the
																		// cell
	...
	void calculateTempIncrement(double sunPosition, int tau) {
		...
		double T_radiation = S * lon_attenuation * lat_attenuation - eps * SB
				* T * T * T * T;
		T_radiation = T_radiation / depth;

		T_inc = (T_radiation + T_neighbor) * tau * 60 / (CW * Ro);

		System.out.println("c_lat =" + c_lat + ":" + "c_lon =" + c_lon + ":"
				+ "sunPosition =" + sunPosition + ":" + " T =" + T + ":"
				+ "T_neighbor =" + T_neighbor + ":" + "T_radiation = "
				+ T_radiation + ":" + "T_inc = " + T_inc
				+ "   neighbor[ 2 ].cell.T =" + (neighbor[2].cell.T));
	}
